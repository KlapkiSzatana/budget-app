...
public class BudgetReminderReceiver extends BroadcastReceiver {
    ...
    static void refreshTasks(Context context) {
        if (!prefs(context).getBoolean("notifications_master", true)) {
            cancelNotification(context, TASKS_NOTIFICATION_ID);
            return;
        }
    ...
    private static String accountBalanceBody(SQLiteDatabase db) {
        ...
        try (Cursor c = db.rawQuery("SELECT id, name, initial_balance FROM accounts ORDER BY id", null)) {
            while (c.moveToNext()) {
                long id = c.getLong(0);
                String name = clean(c.getString(1), "Konto");
    ...
    private static boolean checkMorningBalance(Context context, SQLiteDatabase db) {
        if (!prefs(context).getBoolean("notify_balance", true)) {
            return false;
        }
        Calendar cal = Calendar.getInstance();
        int hour = cal.get(Calendar.HOUR_OF_DAY);
        if (hour >= 12 || !onceToday(context, "balance")) {
            return false;
        }
    ...
    private static boolean refreshPersistentTasks(Context context, SQLiteDatabase db) {
        ...
        SharedPreferences p = prefs(context);
        if (p.getBoolean("notify_bills", true)) {
            hasBills = addBillTasks(db, tasks);
        }
        if (p.getBoolean("notify_debts", true)) {
            addDebtTasks(db, "liabilities", "liability_repayment", "Dług", tasks);
            addDebtTasks(db, "debtors", "debtor_repayment", "Dłużnik", tasks);
        }
        if (tasks.isEmpty()) {
            cancelNotification(context, TASKS_NOTIFICATION_ID);
            return false;
        ...
        for (int i = 0; i < shown; i++) {
            visible.add(tasks.get(i));
        }
        if (tasks.size() > shown) {
            visible.add("I jeszcze: " + (tasks.size() - shown));
        }
    ...
    private static boolean addBillTasks(SQLiteDatabase db, List<String> tasks) {
        boolean added = false;
        Calendar today = calendar(today());
        try (Cursor c = db.rawQuery("SELECT due_date, amount, description, IFNULL(is_recurring,0) " +
                "FROM pending_bills WHERE IFNULL(is_paid,0)=0 ORDER BY due_date, id", null)) {
            while (c.moveToNext()) {
                String due = c.getString(0);
                long days = daysBetween(today, calendar(due));
                if (days > 7) {
                    continue;
                }
                String prefix = c.getInt(3) == 1 ? "Stały wydatek" : "Rachunek";
                tasks.add(prefix + ": " + clean(c.getString(2), "bez opisu") + " " + money(c.getDouble(1)) + " (" + dueLabel(days) + ")");
                added = true;
            }
        }
        return added;
    ...
    private static void addDebtTasks(SQLiteDatabase db, String table, String type, String label, List<String> tasks) {
        Calendar today = calendar(today());
        try (Cursor c = db.rawQuery("SELECT id, name, total_amount, deadline FROM " + table + " ORDER BY deadline, name", null)) {
            while (c.moveToNext()) {
                long id = c.getLong(0);
                ...
                tasks.add(label + ": " + clean(name, "bez nazwy") + " " + money(remaining) + " (" + dueLabel(days) + ")");
            }
        }
    }
    ...
    private static boolean checkDailyExpenseReminder(Context context, SQLiteDatabase db) {
        if (!prefs(context).getBoolean("notify_daily_expenses", true)) {
            return false;
        }
        Calendar cal = Calendar.getInstance();
        if (cal.get(Calendar.HOUR_OF_DAY) < 16 || !onceToday(context, "daily_expenses")) {
            return false;
        }
        ...
        if (count < 1) {
            return notify(context, nextVariant(context, "daily_expense_title",
                    ...
                    "Dzienny przegląd",
                    "Kontrola wydatków",
                    "Budżet na dziś"), nextVariant(context, "daily_expense_body",
                    "Po 16:00 nadal nie ma dziś wpisanych wydatków.",
                    "Dzisiaj nie dodałeś jeszcze żadnego wydatku. Jeśli coś było, dopisz teraz.",
    ...
    private static boolean checkPurchaseGap(Context context, SQLiteDatabase db) {
        if (!prefs(context).getBoolean("notify_insights", true)) {
            return false;
        }
        String threshold = addDays(today(), -2);
        Calendar today = calendar(today());
        try (Cursor c = db.rawQuery("SELECT item, last_date, cnt FROM (" +
                "SELECT CASE " +
                "WHEN TRIM(IFNULL(subcategory,'')) <> '' THEN TRIM(subcategory) " +
                "WHEN TRIM(IFNULL(category,'')) <> '' THEN TRIM(category) " +
                "ELSE TRIM(IFNULL(description,'')) END AS item, " +
                "MAX(date) AS last_date, COUNT(*) AS cnt " +
                "FROM transactions WHERE type='expense' " +
                "GROUP BY item) WHERE item <> '' AND last_date <= ? AND cnt >= 2 " +
                "ORDER BY last_date ASC, cnt DESC LIMIT 12", new String[]{threshold})) {
            while (c.moveToNext()) {
                String item = c.getString(0);
                String last = c.getString(1);
                long days = daysBetween(calendar(last), today);
                if (days < 2) {
                    continue;
                }
                if (today().equals(prefs(context).getString("notified_purchase_gap", ""))) {
                    return false;
                }
                if (!onceEveryDays(context, "purchase_gap_" + stableKey(item), 4)) {
                    continue;
                }
                ...
                return notify(context, nextVariant(context, "purchase_gap_title",
                        ...
                        "Mała obserwacja",
                        "Zmiana w zakupach"), String.format(Locale.ROOT,
                        nextVariant(context, "purchase_gap_body",
                                "\"%s\" nie pojawiło się od %d dni. Może już nie jest potrzebne?",
                                "Ostatni wpis dla \"%s\" był %d dni temu. Jeśli to celowo, budżet właśnie to widzi.",
                                "\"%s\" nie ma w wydatkach od %d dni. Sprawdź, czy to brak potrzeby, czy brak wpisu.",
    ...
    private static boolean onceThisWeek(Context context, String key) {
        String pref = "notified_" + key;
        ...
        p.edit().putString(pref, week).apply();
        return true;
    }

    private static boolean onceEveryDays(Context context, String key, int days) {
        String pref = "notified_" + key;
        String today = today();
        SharedPreferences p = prefs(context);
        if (!hasDailyNotificationSlot(context)) {
    ...
    private static String nextVariant(Context context, String key, String... variants) {
        if (variants == null || variants.length == 0) {
            return "";
        }
        String pref = "variant_" + key;
        SharedPreferences p = prefs(context);
        int next = p.getInt(pref, -1) + 1;
        p.edit().putInt(pref, next).apply();
        return variants[Math.abs(next) % variants.length];
    }

    private static String stableKey(String value) {
        return String.valueOf(Math.abs((value == null ? "" : value.toLowerCase(Locale.ROOT)).hashCode()));
    }
    ...
    private static String dueLabel(long days) {
        if (days < 0) {
            return "po terminie " + Math.abs(days) + " dni";
        }
        if (days == 0) {
            return "dzisiaj";
        }
    ...
    private static String weekdayName(Calendar cal) {
    ...
    private static PendingIntent openActivityIntent(Context context, String screen, int requestCode) {
        ...
        open.setAction(MainActivity.ACTION_OPEN_SCREEN);
        if (screen != null && !screen.trim().isEmpty()) {
            open.putExtra(MainActivity.EXTRA_OPEN_SCREEN, screen);
        }
