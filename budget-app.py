...
class BudgetApp(QMainWindow):
    ...
    def setup_top_bar(self):
        ...
        l.addSpacing(20); l.addWidget(self.search_bar, 1); l.addSpacing(20)
    ...
    def setup_dashboard(self):
        ...
        self.lia_box = QGroupBox(_("Moje Długi (Do oddania):"))
    ...
    def setup_weekly_ui(self):
        ...
        self.lbl_cat_head = QLabel(_("Struktura wydatków:")); self.lbl_cat_head.setStyleSheet("font-weight: bold; margin-top: 10px;")
        self.weekly_cat_area = QScrollArea(); self.weekly_cat_area.setWidgetResizable(True); self.weekly_cat_area.setStyleSheet("background: transparent; border: none;")
        self.weekly_cat_container = QWidget(); self.weekly_cat_layout = QVBoxLayout(self.weekly_cat_container); self.weekly_cat_layout.setContentsMargins(0, 0, 0, 0)
        self.weekly_cat_area.setWidget(self.weekly_cat_container)
        self.weekly_ui_layout.addLayout(nav_layout)
        self.weekly_ui_layout.addWidget(self.lbl_current_limit); self.weekly_ui_layout.addWidget(self.weekly_pbar)
        h_nums = QHBoxLayout(); h_nums.addWidget(self.lbl_weekly_spent); h_nums.addStretch(); h_nums.addWidget(self.lbl_weekly_remaining)
        self.weekly_ui_layout.addLayout(h_nums)
        line = QFrame(); line.setFrameShape(QFrame.HLine); line.setStyleSheet("background: #ccc;")
        self.weekly_ui_layout.addWidget(line); self.weekly_ui_layout.addWidget(self.lbl_last_week_saved); self.weekly_ui_layout.addWidget(self.lbl_month_savings)
        self.weekly_ui_layout.addWidget(self.lbl_cat_head); self.weekly_ui_layout.addWidget(self.weekly_cat_area, stretch=1)
    ...
    def update_weekly_stats_no_data(self):
        self.weekly_widget.setVisible(True); self.chart.setVisible(False)
        ...
        self.lbl_current_limit.setText(_("Brak limitu na ten tydzień")); self.lbl_cat_head.setText(""); self.weekly_pbar.setValue(0); self.weekly_pbar.setFormat("")
        self.lbl_weekly_spent.setText(""); self.lbl_weekly_remaining.setText(_("Użyj 📅 Limit Tygodnia, aby ustalić limit.")); self.lbl_weekly_remaining.setStyleSheet("color: gray; font-style: italic;")
        self.lbl_last_week_saved.setText(""); self.lbl_month_savings.setText("")
    ...
    def update_weekly_stats(self, weekly_enabled, weekly_limit, weekly_cats=None):
        if not weekly_enabled:
            self.chart.setVisible(True); self.weekly_widget.setVisible(False)
            self.weekly_filter_cat = None; self.week_offset = 0; return
        self.chart.setVisible(False); self.weekly_widget.setVisible(True)
        ...
        while self.weekly_cat_layout.count():
            ...
            if item.widget(): item.widget().deleteLater()
        if not cat_data:
            lbl = QLabel(_("Brak wydatków.")); lbl.setStyleSheet("color: gray; font-style: italic; margin-top: 10px;"); lbl.setAlignment(Qt.AlignCenter)
            ...
            for cat, amt in cat_data:
                ...
                btn_name = QPushButton(f"{cat}"); btn_name.setCursor(Qt.PointingHandCursor)
                if is_active: btn_name.setStyleSheet("QPushButton { text-align: left; font-weight: bold; color: #3498db; border: none; background: transparent; }")
                else: btn_name.setStyleSheet("QPushButton { text-align: left; color: palette(text); border: none; background: transparent; } QPushButton:hover { color: #3498db; text-decoration: underline; }")
                btn_name.clicked.connect(lambda _, c=cat: self._handle_text_category_click(c))
                lbl_amt = QLabel(f"{amt:.2f} zł ({cat_pct}%)"); lbl_amt.setAlignment(Qt.AlignRight)
                if is_active: lbl_amt.setStyleSheet("font-weight: bold; color: #3498db;")
                row_l.addWidget(btn_name, stretch=1); row_l.addWidget(lbl_amt)
    ...
    def setup_buttons(self):
        ...
        deb_style = base_style + """ QPushButton { color: #d35400; border-color: #e67e22; } QPushButton:hover { background-color: #fdebd0; } """
        ...
        self.btn_liabilities = QPushButton(_("!! MOJE DŁUGI")); self.btn_liabilities.setStyleSheet(lia_style); self.btn_liabilities.clicked.connect(self.open_liabilities_dialog)
        ...
        self.btns = [self.btn_income, self.btn_expense, self.btn_savings, self.btn_liabilities, self.btn_debtors]
    ...
    def open_context_menu(self, position):
        ...
        if not menu.isEmpty(): menu.exec(self.table.viewport().mapToGlobal(position))

    def setup_footer(self):
        footer_widget = QWidget(); footer_layout = QHBoxLayout(footer_widget); footer_layout.setContentsMargins(10, 5, 10, 5)
        lbl_version = QLabel(f" {WERSJA} "); lbl_version.setStyleSheet("color: gray; font-size: 10px;")
        current_year = datetime.now().year; start_year = 2025; year_str = f"{start_year} - {current_year}" if current_year > start_year else str(start_year)
        copy_text = f"{PRODUCENT} {year_str}"; lbl_copy = QLabel(copy_text); lbl_copy.setStyleSheet("color: gray; font-size: 10px;"); lbl_copy.setAlignment(Qt.AlignRight)
        footer_layout.addWidget(lbl_version); footer_layout.addStretch(); footer_layout.addWidget(lbl_copy)
    ...
    def load_transactions(self, refresh_panel=True):
        ...
        if locked: self.btn_close_month.setText(_("🔒 ODBLOKUJ MIESIĄC")); self.btn_close_month.setStyleSheet("background:#c0392b;color:white;font-weight:bold")
        else: self.btn_close_month.setText(_("🔒 Zamknij Miesiąc")); self.btn_close_month.setStyleSheet("background:#95a5a6;color:white")
        ...
        for r in rows:
            ...
            if show:
                ...
                display_cat = tcat
                display_desc = tsub
                if ttype == 'savings': display_cat = _("Oszczędności"); display_desc = tsub
                elif ttype == 'liability_repayment': display_cat = _("Spłata Długu"); display_desc = tsub
                elif ttype == 'debtor_repayment': display_cat = _("Zwrot od Dłużnika"); display_desc = tsub
                ...
                if ttype == 'debtor_repayment': c = "#d35400"
            ...
            if tdate.startswith(m_str):
                if ttype=="income": stats_inc+=tamt; inc_map[tcat]=inc_map.get(tcat,0)+tamt
                    ...
                    stats_lia+=tamt
                    # Dodajemy to też do exp_map pod wspólną nazwą!
                    cat_name = _("Spłata Długu")
                    exp_map[cat_name] = exp_map.get(cat_name, 0) + tamt

                elif ttype=="debtor_repayment": stats_deb+=tamt

        prev = self.db.get_net_balance_pln_before_date(f"{m_str}-01")
        real_balance = prev + stats_inc - stats_exp - sum(r[5] for r in rows if r[1].startswith(m_str) and r[2]=='savings') - stats_lia + stats_deb
        ...
        else: self.lbl_balance.setToolTip("")
        ...
        if not weekly_view_active and refresh_panel:
            # Scalamy kwotę systemową "Moje Długi" z kategorią wydatku "Spłata Długu"
            # żeby na wykresie był jeden wspólny kawałek tortu.
            debt_cat_name = _("Spłata Długu")
            if stats_lia > 0:
                exp_map[debt_cat_name] = exp_map.get(debt_cat_name, 0.0) + stats_lia

            # Rysujemy wykres. WAŻNE: W trzecim argumencie (liability_amount) dajemy 0,
            # bo dodaliśmy tę kwotę już wyżej do exp_map.
            self.chart.update_chart_pie_app(
                exp_map,
                stats_sav,
                0,         # <--- ZERO (bo długi są już w exp_map)
                stats_deb,
        ...
        self.update_goals_display(); self.update_liabilities_display(); self.update_debtors_display()

    def on_chart_pick(self, event):
        if event.mouseevent.button != 1: return
        try: artist = event.artist; category = artist.get_gid()
    ...
    def open_liabilities_dialog(self):
        ...
        if d.exec():
            dat=d.get_data()
            if dat['mode']=='new':
                # Teraz ta metoda zawsze zwraca True (doda nowy lub zaktualizuje stary)
                self.db.add_liability(dat['name'], dat['amount'], dat['deadline'])
                QMessageBox.information(self,"OK",_("Zapisano zobowiązanie."))
            else:
                self.db.add_transaction(dat['date'], 'liability_repayment', _('Spłata Długu'), dat['name'], dat['amount'])
            self.schedule_update()

    def open_debtors_dialog(self):
        from dialogs import DebtorsDialog
        d=DebtorsDialog(self, self.db)
        if d.exec():
            dat=d.get_data()
            if dat['mode']=='new':
                # Teraz ta metoda zawsze zwraca True (doda nowy lub zaktualizuje stary)
                self.db.add_debtor(dat['name'], dat['amount'], dat['deadline'])

                # Dodajemy wydatek (Pożyczka) - to jest kwota TEJ KONKRETNEJ pożyczki, więc jest OK
                self.db.add_transaction(QDate.currentDate().toString("yyyy-MM-dd"), 'expense', 'Pożyczki', dat['name'], dat['amount'])

                QMessageBox.information(self,"OK",_("Zapisano dłużnika i dodano wydatek."))
            else:
                self.db.add_transaction(dat['date'], 'debtor_repayment', _('Zwrot od Dłużnika'), dat['name'], dat['amount'])
            self.schedule_update()

    def update_debtors_display(self):
        for i in reversed(range(self.debtors_layout.count())): self.debtors_layout.itemAt(i).widget().setParent(None)
        debts = self.db.get_debtors_status()
        if not debts: self.deb_box.hide(); return
        self.deb_box.show()
        today = datetime.now().date()
        for d in debts:
            rem = d['total'] - d['paid']
    ...
    def save_transaction(self, data):
        if self.db.is_month_locked(data['date'][:7]): QMessageBox.critical(self, _("Błąd"), _("Miesiąc zamknięty!")); return
        if data['type']=='income': self.db.add_person(data['cat'])
        self.db.add_transaction(data['date'], data['type'], data['cat'], data['sub'], data['amount'], data.get('exclude', 0))
        self.schedule_update()
    ...
    def gen_rep(self, path, d_str, m_name, ann):
        ...
        for r in tr:
            ...
            elif r[2]=='debtor_repayment': deb+=v
    ...
    def update_goals_display(self):
        ...
        if not goals_data:
            lbl = QLabel(_("Brak celów. Dodaj pierwszy (+)")); lbl.setStyleSheet("font-style: italic; color: grey; font-size: 10px;")
            self.goals_list_layout.addWidget(lbl); return
        for g in goals_data:
            ...
            lbl = QLabel(_("{}: {:.2f} / {:.0f} PLN").format(name, current, target)); lbl.setStyleSheet("font-size: 11px; font-weight: bold;")
            ...
            pbar.setStyleSheet(f"QProgressBar {{ border: 1px solid #bbb; border-radius: 4px; background-color: #f0f0f0; }} QProgressBar::chunk {{ background-color: {color}; border-radius: 3px; }}"); pbar.setTextVisible(False)
